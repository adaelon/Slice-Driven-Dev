...
每个切片完成时，skill 会驱动 Claude 自动维护以下文件：

| 文件 | 更新时机 | 内容 |
|---|---|---|
| `SESSION_CHECKPOINT.md` | 大切片完成 / 会话结束前 | 当前进度、下一步原子动作、未提交项；下次会话冷启动入口 |
| `docs/切片计划.md` | 任务拆分时 | 每个切片的名称、目标、完成判据、进度状态（✅/🅿️） |
| `docs/代码链路.md` | 每个切片完成时追加 | 精确到 `文件:符号` 的改动账本，附入口和测试覆盖 |
| `docs/技术方案.md` | 产生可追溯决策时 | 用固定模板记录：决策一句话 + 否决项 + 何时回头看 |
| `docs/架构.md` | 模块边界变化时 | 组件图、主要数据流、关键决策反向索引 |

这五个文件合在一起，构成项目的完整可追溯链：**切片计划**说要做什么 → **代码链路**说做了什么 → **技术方案**说为什么这么做 → **架构**说整体是什么样 → **SESSION_CHECKPOINT** 说现在做到哪、下一步去哪。
